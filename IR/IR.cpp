 ...
 void IRBuilder::genIR(struct node *T,Symboltable &symboltable) {
    ...
    vector<Opn*> opns;
        ...
        case VarDef:
            ...
            mysymbol.pretype=T->pretype;
            mysymbol.types=T->pretype->getvalue();
            if(T->level==0)//ALLOCA前给符号一个别名
                mysymbol.alias=newGloabl();
            else
            ...
            if(T->pretype->is_BasicType()){
                ...
                if(!opn_type.compare("int")) opn_type="i32";
            }
            else if(T->pretype->is_Array_Type()){
                opn_type=static_cast<Array_Type*>(T->pretype)->basictype.getvalue();
                if(!opn_type.compare("int")) opn_type="i32";
                for(int i=static_cast<Array_Type*>(T->pretype)->lev-1;i>=0;i--){
                    opn_type="[ "+to_string(static_cast<Array_Type*>(T->pretype)->elements_nums[i])+" x "+opn_type+" ]";
                }
            }
            //ALLOCA分配的类型
            opn=new Opn(IDENT,opn_type,T->level,T->offset);
            opn->id=mysymbol.name;
            opns.push_back(opn);
            //ALLOCA等号左边的返回值
            opn=new Opn(IDENT,opn_type,T->level,T->offset);
            opn->id=mysymbol.alias;
            opns.push_back(opn);
            T->code=codegen(ALLOCA,opns);
        ...
        case ASSIGN:
            ...
            //STORE指令
            //目标操作数
            symbol=symboltable.getSymbol(symboltable.Search(string(T->ptr[0]->type_id)));
            if(T->ptr[0]->pretype->is_BasicType()){
                opn_type=T->ptr[0]->pretype->getvalue();
            }
            else if(T->ptr[0]->pretype->is_Array_Type()){
                opn_type=static_cast<Array_Type*>(T->ptr[0]->pretype)->basictype.getvalue();
            }
            if(opn_type.compare("int")==0) opn_type="i32";
            opn=new Opn(IDENT,opn_type+"*",T->level,T->offset);
            opn->id=symbol->alias;
            opns.push_back(opn);
            //源操作数
            symbol=symboltable.getSymbol(T->ptr[1]->place);
            opn=new Opn(INT,opn_type,T->level,T->offset);
            opn->id=symbol->alias;
            opns.push_back(opn);
