...
enum node_kind  {   
    ...
    MulExp, AddExp, RelExp, EqExp, LAndExp, LOrExp, GOTO, LABEL
};
...
struct symbol {  //这里只列出了一个符号表项的部分属性，没考虑属性间的互斥
    ...
    int paramnum;    //flag为'F'时：形式参数个数，flag为'V'或'P'时：数组维数
    int dim[10];     //数组各维数的大小，最多10维数组
    char alias[32];      //别名，为解决嵌套层次使用，使得每一个数据名称唯一
